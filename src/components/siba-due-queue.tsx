...
export function SibaDueQueue({ locale }: Props) {
  const t = useTranslations("siba");
  const [items, setItems] = useState<DueItem[]>([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    fetch("/api/portugal/due-queue")
      .then((r) => r.json())
  ...
  return (
    <Card className="border-blue-200">
      ...
      <CardContent className="space-y-2">
        {items.map((item) => (
          <div
            key={`${item.propertyId}-${item.stayId}-${item.phase}`}
            className="flex flex-wrap items-center justify-between gap-3 rounded-lg border border-blue-100 bg-blue-50/50 p-3"
          >
            <div>
              ...
              <p className="text-xs text-slate-500">
                ...
                {item.foreignGuestCount > 0
                  ? t("dueQueue.foreignGuestCount", { count: item.foreignGuestCount })
                  : t("dueQueue.noForeignGuests")}
              </p>
              <div className="mt-1 flex flex-wrap gap-1">
                ...
                {item.queueStatus !== "prep_window" &&
                  item.queueStatus !== "awaiting_guest_data" && (
                    <Badge
                      variant={item.workingDaysRemaining === 0 ? "destructive" : "outline"}
                      className="text-xs"
                    >
                      {item.workingDaysRemaining === 0
                        ? t("dueQueue.overdue")
                        : t("dueQueue.daysLeft", { days: item.workingDaysRemaining })}
                    </Badge>
                  )}
                {item.queueStatus === "prep_window" && (
                  <Badge variant="outline" className="text-xs text-blue-700">
