...
export function RegionalStayActions({
  propertyId,
  stayId,
  locale,
  system,
  ...
  const loadStatus = async () => {
    try {
      const res = await fetch(
        `/api/properties/${propertyId}/guest-stays/${stayId}/regional`
      );
      if (!res.ok) return;
  ...
  const systemLabel = system === "mossos" ? "Mossos" : "Ertzaintza";
  ...
  return (
    <div className="flex w-full flex-col gap-2">
      ...
      <div className="flex flex-wrap gap-2">
        ...
        <a
          href={`/api/properties/${propertyId}/guest-stays/${stayId}/regional/export`}
        >
        ...
        <Link
          href={`/${locale}/app/properties/${propertyId}/regional-report/print/${stayId}`}
          target="_blank"
        >
          <Button variant="outline" size="sm" disabled={guestCount === 0}>
            {t("stay.printFitxa")}
